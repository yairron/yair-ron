...
```
בונה מחדש את `BTL/ai-summary.txt` (אינדקס קצר בלבד) ואת קובצי התוכן המלא לפי נושא תחת `BTL/ai-content/*.txt`. **עודכן 20.07.2026:** עד לתאריך זה זה היה קובץ ריכוז מאוחד יחיד (~800KB) — פוצל אחרי שג'מיני דיווח בפועל על חסימה בשליפתו (כלי fetch/grounding חיצוניים מגבילים גודל תוכן שהם שולפים מכתובת בודדת). כל עמוד שמסומן כ-`"include"` בתוך `KNOWN_FILES` בסקריפט חייב להיות משובץ גם לנושא אחד בדיוק בתוך `TOPICS` (מילון נפרד באותו סקריפט) — הסקריפט מדפיס אזהרה נפרדת אם עמוד `include` לא משובץ לאף נושא. `ai-summary.txt` עצמו הוא עכשיו רשימה קצרה (כמה KB בלבד): לכל נושא — כתובת מלאה לקובץ התוכן שלו (`https://yairron.com/btl/ai-content/<slug>.txt`), כותרת, תיאור, ורשימת העמודים שבתוכו. הסקריפט סורק בפועל את `BTL/senior_rights`, `BTL/new_immigrants`, ו-`BTL/additional_guides/html`, ומדפיס אזהרה מפורשת אם נמצא קובץ חדש שלא מסווג ב-`KNOWN_FILES`, או קובץ ברשימה שנמחק מהדיסק — **יש להריץ אותו ולבדוק את הפלט בכל פעם שנוסף/מוסר עמוד**, לא רק אחרי עדכון `nii-constants.json`.
...
**רכיבים:**
- `netlify/functions/btl-chat.js` (בשורש הריפו, לא תחת `BTL/`) — Netlify Function שרצה בצד השרת. מקבלת שאלה מהדפדפן, שולפת בזמן אמת את כל קובצי `BTL/ai-content/*.txt` (לפי רשימת `CONTENT_URLS` הקבועה בקוד — ראו הסעיף הקודם), מאחדת אותם לטקסט אחד ומפרקת לסעיפים לפי המפריד `===== path =====` (ראו `build_ai_summary.py`), ובונה מהם **אינדקס קצר בלבד** (נתיב + תיאור קצר לכל עמוד) שנכנס ל-system prompt — לא את התוכן המלא. פונה למודל `claude-haiku-4-5-20251001` (נבחר לפי עלות — מתאים למשימת שאלות-תשובות מבוססת-טקסט כזו). מקבלת רק בקשות שמקורן בדומיין `yairron.com` (בדיקת Origin/Referer). **שים לב:** הפונקציה לא קוראת בכלל את `BTL/ai-summary.txt` (האינדקס הציבורי) — זה מיועד לכלי חוץ כמו ג'מיני, לא לפונקציה הפנימית, שבונה לעצמה אינדקס דומה משלה מתוך הסעיפים שהיא כבר פירקה.
...
3. **קובצי הריכוז לבינה מלאכותית** — אם לעמוד יש תוכן קבוע אמיתי (לא מחשבון/טופס/תוכן תלוי-קלט), להוסיף אותו ל-`KNOWN_FILES` בתוך `build_ai_summary.py` עם `"include"` וסיבה קצרה, ואם לא — עם `"exclude"` וסיבה. בלי זה, `build_ai_summary.py` ידפיס עליו אזהרת "קובץ לא מסווג" בכל הרצה עד שיטופל. **אם הוספת ל-`"include"`** — שני צעדים נוספים, לא אחד: (א) לשבץ אותו לנושא קיים (או חדש) בתוך `TOPICS` באותו סקריפט, אחרת תודפס אזהרת "לא משובץ לנושא"; (ב) להוסיף שורה תואמת ל-`PATH_DESCRIPTIONS` בתוך `netlify/functions/btl-chat.js` (ראו סעיף "עוזר AI (צ'אט) באתר" למעלה), אחרת בוט הצ'אט לא ידע מתי לבחור בעמוד החדש.
