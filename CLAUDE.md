...
```
**למה בכלל צריך את זה, מעבר לפיצול ל-`ai-content`:** פרפלקסיטי וג'מיני דיווחו שהם "מוצאים את האתר ומבינים את הנושא הכללי" אבל לא מצליחים לשלוף פרטים ספציפיים - כי כשהם שולפים עמוד מדריך בזמן אמת הם מורידים את קובץ ה-HTML הגולמי במלואו (כולל כל קוד העיצוב/JS - עד 300KB+ בעמודים כמו `faq.html`), לא רק את הטקסט. המכסה שלהם לתוכן מעמוד בודד נגמרת על קוד העיצוב לפני שהם מגיעים לפרטים בעומק העמוד. `ai-content/*.txt` (הסעיף הקודם) כבר פותר את זה - אבל שום דבר בעמודי ה-HTML עצמם לא הצביע להם שהמשאב הזה קיים.

הסקריפט מוסיף לכל עמוד המשובץ ב-`TOPICS` (ראו הסעיף הקודם) תג `<link rel="alternate" type="text/plain" href="https://yairron.com/btl/ai-content/<slug>.txt">` מיד אחרי ה-`meta description` ב-`<head>` - תקן רשת סטנדרטי (אותו מנגנון כמו גילוי RSS) שמצביע על גרסת טקסט נקייה של אותו עמוד, בלי לשנות שום דבר בתצוגה לבן אדם. **אידמפוטנטי** - מריצים אותו שוב בלי סיכון, הוא מדלג על עמוד שכבר יש בו את התג. **יש להריץ אותו אחרי כל הרצה של `build_ai_summary.py` שמוסיפה/מסירה נושא או עמוד**, כדי שהקישורים יישארו מסונכרנים עם `TOPICS`.
